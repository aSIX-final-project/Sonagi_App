    ...
    const fetchData = async () => {
      try {
        let response = await axios.get(
          "https://port-0-sonagi-app-project-1drvf2lloka4swg.sel5.cloudtype.app/boot/Ability/findAll"
        );
    ...
    <View style={styles.container}>
      <View style={styles.topView}>
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate("ManagePage", { userInfo: userInfo })
          }
        >
          <Image
        ...
        <Text style={styles.title}>재능기부 요청 관리</Text>
      </View>
              ...
              <View style={styles.row}>
                <Text style={styles.requestTitle}>
                  {item.introduction === null
                    ? `${item.adName} 팀 재능기부 요청`
                    : `${item.adName} 팀 재능기부 요청`}
                </Text>
              </View>
              <Text style={styles.date}>{item.adTel}</Text>
